...
def help_message():
    return ('<b>Управление торговым циклом</b>\n'
            'Одна команда запускает от одного до пяти последовательных циклов по локальной конфигурации.\n\n'
            '<b>Просмотр</b>\n'
            ...
            '<code>/run ack 1 5</code> — пять циклов подряд: ACK, улучшение на 1 тик.\n'
            '<code>/run 5</code> — пять циклов с настройками по умолчанию.\n'
            'Число циклов — 1–5; без него запускается один. Режим и улучшение на 1–5 тиков '
            'действуют для каждого цикла серии и его парного закрытия. Следующий цикл требует '
            'завершённого результата, нулевых позиций и новой проверки; ошибка останавливает серию.\n\n'
...
def unknown_message():
    return ('<b>Команда не распознана</b>\n'
            ...
            'Пример серии: <code>/run ack 1 5</code> (не более пяти циклов).')
