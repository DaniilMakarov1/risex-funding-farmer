...
The default product is **paper only**. It uses official public RISEx, Extended, and Nado market data. Separately governed optional testnet modules cover the accepted RISEx account bootstrap and accepted session-signer prerequisite; the preserved signer is operationally active. The RISEx fixture lifecycle core is centrally accepted, while its sole next step is a separately gated credentials/private-read preflight; Extended retains its independently bounded lifecycle scope. The Nado fixture lifecycle core is also centrally accepted, and its next governance-only follow-up proposes one isolated credentials/private-read preflight inside the same `NADO-TESTNET-001` milestone. This governance candidate itself has no Builder, credential, private-network, real-signature, private-POST, or live-write authority. Mainnet, real funds, operational orders/positions, and strategy-driven execution remain outside the active specification.
...
`TESTNET-002-RISEX-ORDER-LIFECYCLE-001` is also not a CLI mode and must not be imported by normal Farmer startup. Its fixture core for one exact-minimum BTC testnet lifecycle is centrally accepted at `4b323c6d` through published merge `998b0c3`. The next bounded step remains inside that lifecycle: two agreeing official public preflight sweeps, followed only after a separate Chief Coordinator operational gate by one existing-signer load, one nonce/`RegisterV2` `auth_v2` attempt, and one private `orders` plus one `positions` snapshot. The conflicting legacy WebSocket contract is never tried as a fallback. This governance candidate authorizes no credential/XLSX access, signature, private request, nonce, Builder, or live write. Operational success remains zero open orders plus exact flatness; preflight success alone is not lifecycle, exact-flat, operational, or strategy acceptance. The narrow user-accepted later live-test risk may end `FAILED_HALTED_MANUAL_RECOVERY`; that failure is likewise never operational acceptance or strategy readiness.
...
`NADO-TESTNET-001` is not a CLI mode and must not be imported by normal Farmer startup. Its fixture lifecycle implementation through `03ccbe118840e62cbd9f6e9b8e08ee46248be3b2` is centrally accepted through merge `9762a625f16262c5b4ac1dfd6036499e0ef2ab3`; it remains synthetic, isolated evidence for durable identity, no-blind-retry reconciliation, bounded recovery, and the full-catalog zero-orders/exact-flat barrier. The active follow-up is governance-only for a future minimal private-read preflight: fixed Ink Sepolia/Endpoint/owner/subaccount identity, two agreeing live engine rounds, zero linked signer, complete zero regular/cross/isolated state, and one unfiltered signed `ListTriggerOrders` read that must return no history. The observed official test-gateway HTTP `403` is a hard operational stop unless standard-host public reads succeed from the user's already-attested lawful non-restricted connection; proxy/VPN circumvention is prohibited. A separate Chief Builder gate is required before implementation, and a further Chief credential/private-call gate is required before any owner-key load, real signature, or signed read-only POST. No live write, order/cancel/close, deposit, linked signer/API key, shared paper-runtime change, or operational-readiness claim is authorized.
