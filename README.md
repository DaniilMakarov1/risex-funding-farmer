...
FINAL-ASTRA-01 (2026-09-06) authorizes Scanner v1; **S1a is accepted, the full scanner is not yet complete**. The accepted versioned kernel subset adds venue-local quantity/minimum rules, retained partials and exact notional, four fill-model/delay alternatives and activation post-only checks. Historical default behavior remains separate. Exact SHA/evidence and limitations are in STATUS. The next task is SCV1-S1b; its first useful new-policy offline D1 report must precede new collection. SYSTEM_SPEC's SCV1-1 is the new versioned policy, not a claim that the commands below already implement it.
