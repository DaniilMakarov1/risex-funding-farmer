...
## First confirmation-gated agent Mainnet attempt — HCR-9/HCR-10

The launcher is under implementation and independent review; no HCR-9 live attempt has been verified. HCR-10 establishes a confirmation-gated workflow. For each proposed live operation, the assigned execution agent must show the exact venue/accounts/market, sides and order types, quantities, price/time bounds, maximum writes, expected resulting positions and permitted recovery, then ask the owner to confirm that plan. An affirmative reply authorizes one execution of that displayed plan. Changed inputs/state, completion, cancellation or another operation requires another plan and confirmation; there is no standing trading mandate.
...
Help and execution preview remain offline and do not access Keychain even when `--keychain` is present. Builder verification uses synthetic keys/backends; real user keys and actual OS Keychain storage are not accessed in those tests. HCR-10 separately permits the assigned execution agent to use real credentials after the applicable finite plan is accepted and the owner confirms that displayed plan. The current configured Robinhood endpoint is real Mainnet, not a valueless test environment.
...
With deferral enabled, the four source/receiver incremental-margin estimate/provenance fields may be omitted from market evidence. Current market identity/grid/minimum evidence and its original timestamp remain required. This command does not choose missing prices or deadlines. Existing execution/plan-review switches and optional `--keychain` remain separate; the new flag alone never enables execution. Under HCR-10 the assigned execution agent must render the exact finite Mainnet plan, ask for confirmation and may execute it only after the owner affirmatively confirms that plan. The owner's test budget is on Mainnet; no testnet migration is intended.
...
The owner's selected orientation is receiver LONG: A27331 opens SHORT and B27337 opens LONG. Both use API key index4 with separate keys. Keys may be entered through hidden local input/Keychain or supplied to the assigned agent in the private owner-agent task/chat under HCR-10; never place them in this file, Git/GitHub, configs, logs or evidence. Before the first attempt both selected-market positions must be exactly zero and readiness must be proven. This active HCR-9 plan covers one attempt; any later slice or operation needs its own active contract, rendered plan and confirmation. Internal `expected_source_position`/`expected_receiver_position` fields are not operator settings and are rejected in JSON.
