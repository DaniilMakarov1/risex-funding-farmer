...
Current development (2026-09-12): HCR-1 is a separately opt-in Lighter HOOD close/reopen utility, not yet implemented or accepted. The requested operation reduces Q at account A and opens Q in the same direction at account B at a new entry price. It cannot promise the accounts trade directly with each other or execute atomically. Original entry price/PnL does not transfer. NEXT_TASK defines the finite offline development contract; agents have no authority to access accounts, handle credentials or execute orders. Required quantity, prices and risk bounds will be supplied explicitly by the operator. Runnable commands and installation instructions will be added only after a reviewed implementation exists; the research commands below do not perform this operation.
