...
Use `/run` to choose one LIMIT with one or several MARKETs, then the cycle count. Typed `/run ack 1 20` remains one LIMIT with one MARKET. From the terminal use `./start --receiver-admission ack --price-improvement-ticks 1 --fanout` for several MARKETs. WS admission is disabled in owner commands. An old saved `ws_confirmed` default is read as ACK without changing the configuration file or resetting the bot's bound state; `/run ws` and an explicit WS option are refused. The WebSocket connection still supplies current account/order/book observations used by ACK.
...
The new LIMIT→MARKET measurement includes the wait before the actual send. Old journal cards can still show the earlier intent-based measurement. An offline benchmark with an added 50 ms per disk flush reduced the three-receiver ACK→first MARKET interval from about 296–303 ms to 58–61 ms (five waits to one); the full synthetic cycle uses 18 flushes instead of 59. This is a synthetic storage result, not an exchange latency or own-match guarantee. Saved fan-out cycles 341–354 have seven PARTIAL and seven SUCCESS outcomes, all with confirmed flat final positions; ACK does not guarantee that our LIMIT wins against outside participants.
...
Now an order that was prepared but never sent always gives its number back, so the next attempt or the reduce-only close can proceed. A close or open attempt that provably sent nothing is no longer "unknown": after a timeout it is simply tried again after a short pause (within the usual attempt limits), and otherwise the positions are closed with reduce-only orders. The cycle card no longer says the leftover was closed when one account is still open, and an account without a position reads "позиции нет". Cycle-314 (the MARKET order filled against another trader, leftover closed, flat) is normal market behavior.
