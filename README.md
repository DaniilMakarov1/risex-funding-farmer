...
The short Russian output now separates these facts: whether the receiver was never sent, whether a fallback dispatch was accepted, whether it filled fully, partially or not at all, and when the displayed source and receiver positions were last observed. The complete sanitized order, mismatch map, trade history and account evidence remain in JSONL. This change does not widen prices, weaken identity checks, add a new retry limit or guarantee a fill.
...
**Historical owner result:** `operator-v1/cycle-001`, `cycle-002` and `cycle-003` are consumed and must never be cleared or reused. The HCR-21 read-only diagnosis observed source27331 SHORT0.00023BTC and receiver27337 zero with no active BTC orders around timestamp1789892127.81; this is historical evidence, not a current-flatness claim. No position was changed by the software correction. A new opening still requires both accounts exactly flat with no active BTC orders.
...
- `cycle-001`, `cycle-002` and `cycle-003` are consumed evidence. Preserve them permanently. The launcher atomically chooses the next unused owner-only `cycle-NNN` directory and a unique client-order prefix after confirmation.
...
The HCR-21 read-only incident check observed source-.00023BTC and receiver0 around timestamp1789892127.81 after the cycle-003 fallback had canceled without a fill. That observation can become stale and does not prove current inventory. The launcher does not adopt or close historical inventory and refuses a new opening while selected-market positions or pending orders remain. All original owner attempts remain immutable.
...
In automatic execution mode, first review the fixed accounts, quantity, direction, timing and price-selection rule; price is still unresolved. Type `LAUNCH` once to approve those parameters and automatic price selection. Any other response cancels offline without Keychain/private access. Both credentials then load through Keychain/hidden input, after which fresh metadata/book observations determine the exact source and receiver price and displayed notional. No agreement with an earlier quote is required because no quote was collected before the prompt. There is no second price confirmation, automatic retry or quantity adjustment.

Actual stale, invalid, crossed or wrong-market data still stops the operation before engine admission. A known pre-execution stop does not prove current account-wide positions are flat. Missing engine result remains incomplete; a failure after engine admission must be diagnosed from the preserved packet/journal. Enter keys only at the hidden-key prompt.
...
Help and execution preview remain offline and do not access Keychain even when `--keychain` is present. Builder verification uses synthetic keys/backends; real user keys and actual OS Keychain storage are not accessed in those tests. HCR-10 separately permits the assigned execution agent to use real credentials after the applicable implementation is accepted and the owner confirms the proposed live action. The current configured Robinhood endpoint is real Mainnet, not a valueless test environment.
...
The owner's selected orientation is receiver LONG: A27331 opens SHORT and B27337 opens LONG. Both use API key index4 with separate keys. Keys may be entered through hidden local input/Keychain or supplied to the assigned agent in the private owner-agent task/chat under HCR-10; never place them in this file, Git/GitHub, configs, logs or evidence. Before the first attempt both selected-market positions must be exactly zero and readiness must be proven. This active HCR-9 plan covers one attempt unless the owner confirms a broader or later action. Internal `expected_source_position`/`expected_receiver_position` fields are not operator settings and are rejected in JSON.

The requested first test is roughly10–15USD of BTC exposure per leg, not margin or transferred balance. Quantity inputs are BTC units; check current price, size step and minimums before converting the desired notional. Do not round up beyond the chosen exposure simply to satisfy a minimum unless the owner confirms that change. The slice logic, price bounds and stop/reconciliation behavior described below still apply. Source filling before B's admission blocks B; market matching and cancellation can race, leaving unequal positions that require operator attention. The program does not automatically compensate or close those positions.
...
Current status (2026-09-15): HCR-1 BOTH/no-monetary-caps implementation `74fbbbf9f298648b643ce9a69353e3abcdcf6c27` passed independent review, 12 adverse fake-SDK probes and the clean isolated Python 3.11 suite (4289 passed, 3 skipped). Operator instructions follow. Live execution remains NOT_RUN. HCR-10 now establishes owner confirmation for an assigned execution agent; the separate active HCR-9 Robinhood paired-opening plan is its first application and this does not retroactively authorize the older HOOD example.
