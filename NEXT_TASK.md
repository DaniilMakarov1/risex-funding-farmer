...
## RESUMED owner-authorized objective

The owner explicitly resumed HCR-21 on 2026-09-20. Builder work, offline tests, independent review, integration, installation and publication may continue within the existing objective. No new live read, cycle or mutation is authorized by this resume. Preserve all branches, worktrees, commits, journals and evidence exactly.

Recovered checkpoint: accepted `main`, Chief and remote are `8deb2660e05879e214e95a73c2b01fb0213b0dc4`. Two independently produced, unaccepted clean candidates are preserved: `a4037fa8126912dad000b8bdc7d365a2bb5fde60` in `codex/spread-v1-fallback-reconcile-builder` and `ff5309110fe8d27b32100d6287978eb94b7ff522` in `codex/spread-v1-fallback-reconcile`. Chief review rejected the first because its rejected-dispatch read path can reference `after` before assignment and its helper no longer guarantees a non-empty order ID. The second retains the stronger behavior and is the correction base. Its same healthy Builder resumed only the two finite corrections: reject an empty observed order ID through the actual poll path when an accepted receipt has no order ID, and preserve durable position values/timestamps in final plus exception/cancellation Russian output. Do not start a third implementation or merge either current candidate before a corrected result and independent acceptance.
