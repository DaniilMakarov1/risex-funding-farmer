...
This correction is finite and limited to the following defects:

1. Recheck the guarded public book's age at the final receiver-admission boundary after the exact source lookup. Include its original `observed_at` in the single monotonic mutation deadline together with the account, exact source-order and metadata evidence. The same deadline must cover nonce acquisition, signing and `sendTx`. A book that was fresh on receipt but expires before admission must not create a receiver dispatch intent; a deadline crossed after intent but before transmission must remain a non-replayable ambiguous/rejected mutation and must never call `sendTx`. Add deterministic tests for both boundaries, including the reproduced 9.9 s + 0.2 s case under a 10 s freshness limit.
2. Make the terminal explanation distinguish the original paired-execution failure, ordered fallback recovery history and current inventory. For the exact cycle-004 facts, report that the receiver filled against external account 16969 while the source remained zero-fill/canceled, fallback attempt 1 terminal-zero-filled, fallback attempt 2 fully closed the residual, final inventory is confirmed flat, fees remain UNKNOWN and boundary books are unavailable. Never present an earlier residual as the current state after a later proved full close. Replace the self-authored string assertion with a regression that calls the production formatter/classifier on the saved fact shape.
3. Classify whole-cycle economics from every actual execution, including fallback trades. A fallback fill with any missing fee must keep cycle economics `UNKNOWN` even when opening/closing economics are `PROVEN`. A zero-fill fallback adds no execution fee requirement; known fallback economics requires complete reconciled history and a fee value for every fallback trade. Retain bounded economic status/evidence in `FallbackResult`, its durable serialization and terminal result without inventing absent fees.

Do not change price selection, quantity/hold sampling, attempt budget, freshness threshold, polling, slippage, minimum, balance, fallback or live-execution policy. Required verification: direct regressions for all three defects; existing priority, no-replay, reconciliation, exact-flat and cycle-004 regressions; complete changed-file review; and one final clean isolated Python 3.11 full suite on the exact candidate. No live validation, account/network/Keychain access, signing, order or cancellation is authorized.
