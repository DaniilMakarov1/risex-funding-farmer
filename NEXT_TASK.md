...
Status: `BLOCKED — NADO LIVE FUNDING ADAPTER IS ACCEPTED, BUT THE FIRST POST-ACCEPTANCE READ-ONLY PREFLIGHT ENDED TERMINAL HTTP ON PUBLIC CONTRACTS; EXTENDED TESTNET BOOKS HAVE ZERO ASKS AND A STALE FUNDING TIMESTAMP; NO WRITE IN FLIGHT`.
...
- Post-acceptance one-shot invocation `nado-read-1d96961ac732670bb0b60490da9049f2` completed identity/signature/zero-trigger checks and the first four public round-B reads, then ended terminal `HTTP` on the fifth read, public `contracts`. Its durable state is `UNKNOWN/VALIDATION_FAILED` with SQLite integrity `ok`; do not resume or retry that invocation. A separate fresh bounded read-only gate is required before any order preparation or dispatch.
