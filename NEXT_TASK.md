...
Status: `LEVEL C BLOCKED BEFORE WRITE ON SEQUENTIAL REST FRESHNESS`.
...
- Open one fresh Luna-max Builder from current `main` to make the Extended observation genuinely fresh for every write-critical field. A bounded venue-local reorder or split observation is allowed; increasing the five-second limit, relabeling local timestamps, dropping required unrelated-state/reconciliation reads, changing signing/write identity/dispatch/no-replay/final barriers, or touching shared core and other venues is forbidden. Require production-shaped delayed-transport regressions showing preparation rejects stale book/state and accepts only truly fresh evidence.
