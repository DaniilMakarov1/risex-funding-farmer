...
Status: `BLOCKED — NADO HAS NO ACCOUNT FUNDING RECORD TO PROVE EVENT AGREEMENT; EXTENDED TESTNET BOOKS HAVE ZERO ASKS AND A STALE FUNDING TIMESTAMP; NO WRITE IN FLIGHT`.
...
- Separate bounded diagnostics established that the preceding Nado `SCHEMA` results came from local diagnostic decoder mistakes, not a venue response contradiction. The accepted catalog decoder then observed 94 products and 79 perps; the official archive exposed the documented account-history cursor contract and two realized hourly rate rows for product 44. The exact protected subaccount had no funding-payment row and `next_idx=null`, so applied account cash and exact product/timestamp/rate event agreement remain unproven. Treat this as `NO_ACCOUNT_FUNDING_RECORD`, never zero; do not create the live-adapter Builder or prepare an order until a fresh bounded gate can observe an authoritative account-scoped record that agrees with realized event history.
