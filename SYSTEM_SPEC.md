...
Status: DESIGN_AUTHORIZED, NOT_IMPLEMENTED_OR_ACCEPTED. This separately opt-in utility implements the owner's requested Lighter HOOD position close/reopen: A is the owner's account and B the friend's account. The owner accepts reducing an agreed quantity at A and opening the same quantity in the same direction at B at the new execution price. This is not a native position transfer: original entry price and PnL are not preserved. No direct-counterparty matching, two-account atomicity or completed exposure handoff is guaranteed. Historical research policy and default entrypoints remain unchanged.
...
The utility is HOOD perpetual only. Validate environment, current market identity/status, account identities, independently authorized control of both accounts, exact positive quantity, direction, tick/size precision, operation minimums and current source position before any mutation. Source A must hold at least Q in the chosen direction. Receiver B must be flat or already in that direction; an opposite position would be a reduction, not the requested opening, and fails preflight. Reject same-account identities, stale/ambiguous state, conflicting active HOOD orders or inability to establish required preconditions. Required configuration includes Q, A limit price, B worst acceptable price, maximum gross notional and fee budget per account, metadata/account freshness bound, finite request/order/reconciliation deadlines and polling bounds. Reject missing or invalid bounds. Fees and minimums require documented/current evidence; do not assume scanner zero fees or minimum exemptions. Insufficient margin or unproven readiness must block safely.
