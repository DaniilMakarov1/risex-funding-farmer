...
Only officially documented onboarding is implementable. The RISEx testnet faucet/deposit may be exposed as one explicit unsigned, one-shot operation followed by the official token-balance postcondition. Nado signed onboarding remains fail-closed when the official contract cannot be used safely without dependency workarounds. Extended signing and lifecycle fixtures are governed separately by section 23 and do not enter TESTNET-001. CI is fixture-only. Real secrets, account writes, faucet/deposit calls, and private live queries remain prohibited until Chief accepts the deterministic candidate and separately authorizes the operational step.
