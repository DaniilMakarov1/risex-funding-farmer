...
Durable evidence records bounded BBO, exact source level and order identity, external better/same-price evidence, priority status/reason, request and observation times, and phase durations. Terminal reporting independently classifies paired execution, inventory and economics. Missing fee evidence remains `UNKNOWN`; later successful fallback must replace stale residual reporting while retaining the original cause. Existing price selection, quantities, fees, slippage, freshness, polling, minimums, balances and risk policy are unchanged.
