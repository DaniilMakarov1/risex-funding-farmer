...
On 2026-09-15 the owner explicitly removed the blanket prohibition on agent mainnet order execution and on API-key exchange between the owner and an assigned execution agent. The owner subsequently selected confirmation-gated execution: an agent may propose a concrete live plan, ask for confirmation and, after the owner affirms that displayed plan, execute it. This amendment supersedes earlier owner-local-only, agent-offline-only, no-agent-execution and no-credential-in-task/chat language only through this confirmation procedure and the active contract in NEXT_TASK. Historical statements remain true for the runs they describe and do not grant authority to repeat them.
...
Before every live operation, the assigned agent must display an exact finite plan covering venue/environment, accounts, market, side/direction, order types, quantities, price bounds, time bounds, maximum writes, expected resulting positions and permitted recovery. The agent must directly ask the owner to confirm that plan. An affirmative reply authorizes one execution of only that displayed plan plus its listed cancellation/reconciliation path. Any material input or account-state change requires a revised plan and new confirmation; completion, cancellation or another operation always requires another confirmation. Ambiguity stops new writes and preserves reconciliation/no-replay semantics. No standing mandate, implicit retry, unrelated cancel, transfer, withdrawal, leverage/margin-setting change or autonomous strategy is created.

The first active application remains HCR-9: one Robinhood Chain Lighter Mainnet PAIRED_OPENING attempt after its implementation is accepted and exact inputs are fixed. Later confirmed operations must first be recorded as the finite active contract in NEXT_TASK; old confirmations and historical commands confer no authority. This documentation amendment itself performs no live access or order.
