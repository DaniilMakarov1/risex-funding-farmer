...
## 21. TESTNET-001 account bootstrap exception

The user explicitly authorizes a separate, opt-in testnet-only account bootstrap and read-only connectivity path for RISEx, Nado, and Extended. It is isolated from the paper runtime, Scanner, lifecycle, economics, Telegram, and persistence. It never targets mainnet, handles real funds, places/cancels/replaces orders, creates positions, or enables strategy execution.

`TESTNET-001-RECOVERY-001` corrects the rejected bootstrap ownership boundary. The connectivity module that allowlists the exact official testnet host, path, method, chain, signing domain, and wallet also owns DNS/TLS dispatch, disables redirect following and environment proxy inheritance, validates the actual response URL, applies finite timeouts, and closes its session. Its public API exposes no sender, transport, session, URL, or base-URL substitution. A successful write response is not readiness: an authoritative module-owned account read must prove the expected wallet state. Ambiguous write outcomes are never retried automatically or reported ready.

Only officially documented onboarding is implementable. The RISEx testnet faucet/deposit may be exposed as one explicit unsigned, one-shot operation followed by the official token-balance postcondition. Extended and Nado signing and lifecycle fixtures are governed separately by sections 23 and 24 and do not enter TESTNET-001. CI is fixture-only. Real secrets, account writes, faucet/deposit calls, and private live queries remain prohibited until Chief accepts the deterministic candidate and separately authorizes the operational step.

The user has authorized a future, separately governed testnet sequence for minimum-size order observation, cancellation, fill/position reconciliation, and reduce-only close. That future authorization does not add trading methods to TESTNET-001 and does not authorize mainnet or real funds.

Official RISEx exposes no authoritative registration-state read, and the never-funded approved wallet's balance endpoint returns only an exact generic internal-error tuple. `TESTNET-001-RECOVERY-005` may therefore consume one fixed non-secret first-deposit authorization stored directly in the already-existing passwd-resolved home directory as `.risex-funding-farmer-testnet-first-deposit-v1.json`. The optional bootstrap module exclusively creates it with no-follow/exclusive semantics, durably flushes canonical `SPENT_UNKNOWN` state and the held home directory before the only POST, and never resets, deletes, rearms, or retries it. Existing or uncertain local state always blocks another POST. Local `READY` is only a hint and requires a fresh authoritative positive balance. Ordinary concurrency, restart, crash, cancellation, and network ambiguity are in scope; intentional same-UID deletion or mutation is operationally prohibited but outside the threat model.

## 22. TESTNET-002 RISEx session-signer prerequisite

The user explicitly authorizes a separately governed RISEx testnet order/cancel/close sequence. Before any order primitive, `TESTNET-002-RISEX-SIGNER-001` must create, register, and authoritatively verify one short-lived session signer for the fixed approved testnet wallet. It is isolated from paper runtime, Scanner, lifecycle, economics, Telegram, orders, positions, and strategy.

The optional signer module owns the fixed `https://api.testnet.rise.trade` destination, chain `11155931`, exact current EIP-712 name/version/Authorization contract, wallet mapping, official bitmap nonce choice, typed-data construction, secret redaction, one registration dispatch, authoritative status/list reconciliation, and one fixed non-secret passwd-home operation record. The account key signs `RegisterSigner(address account,address signer,string message,uint32 expiration,uint48 nonceAnchor,uint8 nonceBitmap)`; the generated session key signs `VerifySigner(address account,uint48 nonceAnchor,uint8 nonceBitmap)`. Both use the same official domain and nonce pair and are submitted as 65-byte `0x` hex signatures. A fresh anchor (`current + 1`) at bitmap index `0` and the official 30-day practical expiration example are fixed for this probe prerequisite.

The session key is generated with a cryptographically secure vetted library only after deterministic acceptance and stored outside the repository in one fixed current-owner `0600` credential file. The main wallet key remains in the protected external source and is loaded only after all public identity/nonce/expiration checks; its derived address must exactly match the approved wallet before signing. The Builder uses synthetic keys and disposable paths only.

Registration is disarmed by default, explicitly invoked once, durably consumed before dispatch, and never blindly retried. Re-entry first checks the exact public signer address; authoritative active status produces zero POST. Timeout, disconnect, cancellation, crash, or ambiguous response permits only bounded read-only status/list reconciliation. Readiness requires authoritative status `1` for the exact account/signer and consistent active list evidence, never a POST body or local file alone. Revocation typed-data/request construction must be fixture-tested before onboarding, but successful onboarding does not revoke and this slice exposes no live revoke dispatch. No order/place/cancel/position method is permitted.

## 23. EXTENDED-TESTNET-001 fixture-only bounded lifecycle core

Extended Phase 0 is accepted for one isolated, fixture-only testnet lifecycle module around official Python SDK commit `2130cdb1cd6e7b1867db83bd3af036572d258739`. The authoritative testnet configuration is `https://api.starknet.sepolia.extended.exchange/api/v1` with Starknet domain `Perpetuals`, version `v0`, chain ID `SN_SEPOLIA`, revision `1`. This exception is not imported by normal Farmer startup and does not change Scanner, runtime, economics, strategy, Telegram, paper persistence, mainnet, or real-funds boundaries.

Before any hypothetical dispatch the module must durably persist one `PREPARED` intent containing an explicit unique nonce in `1..2^31-1`, the SDK settlement-hash external ID, a canonical payload digest, and expiry. The SDK default nonce generator and reuse of any consumed, rejected, cancelled, filled, expired, or ambiguous nonce/external ID are prohibited. Only price-bounded `IOC` orders with short explicit expiry are in scope; `GTC`, `GTT`, `FOK`, and `TOB` are excluded from this slice.

Fixture-tested preflight must require an active non-RFQ perpetual, `minOrderSize == minOrderSizeChange`, exact tick/step alignment, current two-sided depth for one minimum step, worst-price entry and close notionals each `<= USD 500`, available account balance/fee/leverage evidence, and agreeing comprehensive account-stream plus REST evidence of zero open orders and exact flatness. A current public book snapshot is never an exit guarantee and absence of an eligible market forbids dispatch.

Ambiguous place, cancel, mass-cancel, or close is never blindly retried. Reconciliation uses the exact persisted external ID, order status/history, fills, authoritative position, expiry, server-generated stream time, and a fresh comprehensive account snapshot. Close is an opposite-side, price-bounded `IOC` reduce-only order for the exact authoritative position size with a new persisted identity. Any off-grid or below-minimum residual, stream gap, inconsistent REST/stream evidence, signature/identity mismatch, or unresolved rejection fails closed without rounding or another write.

Final success requires server time beyond every dispatched expiry, no pending unexpired intent, a fresh gap-free comprehensive account snapshot, agreeing REST evidence of zero open orders and zero positions, and all observed fills reconciled to persisted lifecycle identities. CI uses fixtures only. The Builder receives only synthetic keys and disposable storage; API/Stark keys must never be logged or persisted. Credential access, authenticated/private network calls, private preflight, signing with real testnet keys, deposits, and every live POST remain outside `EXTENDED-TESTNET-001` and require later separate Chief Coordinator gates.

## 24. NADO-TESTNET-001 fixture-only bounded lifecycle core

Nado Phase 0 is accepted for one isolated, fixture-only testnet lifecycle module translated minimally into Python from official Nado-owned TypeScript SDK commit `315e4f23dadefeb2f86f713e423241e81467d4c3`, Rust SDK commit `e54118786b171a4325871d5bd17e5abae0e90c5a`, and contracts commit `11c27b2851999f1b4f8cb4a7fbfcc9320253f12f`. The authoritative environment is Ink Sepolia chain `763373`, EIP-712 domain `Nado` version `0.0.1`, current Endpoint `0x698D87105274292B5673367DEC81874Ce3633Ac2`, gateway `https://gateway.test.nado.xyz/v1`, gateway WebSocket `wss://gateway.test.nado.xyz/v1/ws`, archive `https://archive.test.nado.xyz/v1`, and trigger service `https://trigger.test.nado.xyz/v1`. No JavaScript service, SDK runtime, framework, or dependency workaround is added. This exception is not imported by normal Farmer startup and does not change Scanner, runtime, economics, strategy, Telegram, paper persistence, RISEx, Extended, mainnet, or real-funds boundaries.

The lifecycle uses one dedicated owner EOA and one fixed subaccount identity, never a linked signer or API key. Before any hypothetical dispatch it must durably persist the canonical payload, exact EIP-712 digest, product identity, endpoint/domain identity, signed `recv_time`, and unique 44-bit `recv_time` plus 20-bit salt nonce or cancellation `tx_nonce`. Fixture signing uses synthetic keys only and must reproduce the pinned SDK/source digest, signature, and signed `validate_order` contract. The same digest is never replayed: official duplicate-digest rejection does not resolve an ambiguous first dispatch, so every ambiguity is fenced by the persisted identity, expiry/`recv_time`, and fresh authoritative engine state rather than a blind retry.

Fixture-tested preflight must dynamically read the complete current product catalog and require the exact chain/domain/Endpoint, active perpetual metadata, exact x18 price/amount/tick/step/minimum/notional conversion, fresh bounded price evidence, every potential entry or close notional `<= USD 500`, zero regular and trigger orders, every cross-perpetual balance amount exactly zero, and no isolated position. The sole entry primitive is a minimum-size price-bounded post-only order with its signed `validate_order` fixture. Acceptance and every partial-fill outcome are reconciled by the exact persisted digest against authoritative engine order/account/position state; archive matches and transaction hashes are audit evidence only.

Recovery first uses `cancel_product_orders` with an empty product list for regular-order cancel-all across the complete catalog. An ambiguous place, cancel-all, or close is never resent with the same digest or nonce and never causes a blind state-changing retry. After fresh authoritative snapshots and after the preceding signed request can no longer arrive within its recorded `recv_time`, a nonzero authoritative position may be closed only by a new opposite-side, aggressive-limit IOC reduce-only order. Official oversize reduce-only clamping may use the product minimum to close a below-minimum residual only when the signed worst-price notional remains `<= USD 500`. Partial fills, nonce/digest reuse, stale or contradictory state, off-grid quantities, rejected close, or a residual not covered by that clamp permit at most three total state-based close attempts, each with a new durable identity after a fresh snapshot; exhaustion halts for a manual gate and is never success.

Final success requires all persisted digests and fills to reconcile, no pending ambiguous or unexpired intent, zero regular orders across the complete product catalog, zero active trigger orders, every cross-perpetual balance amount exactly zero, and no isolated position in fresh authoritative engine state. CI and all Builder verification are fixture-only with synthetic keys and disposable storage. This milestone contains no credential loader invocation, private or live network request, real signature, faucet/deposit, account creation, live POST, order, cancel, fill, close, or position change. Every credential, onboarding, private-read, live-write, exact-flat, and operational-readiness step requires a later separate Chief Coordinator gate.

## 25. TESTNET-002-RISEX-ORDER-LIFECYCLE-001 fixture-only bounded empirical lifecycle

The user accepts the irreducible execution risk of one later separately gated RISEx testnet empirical lifecycle. General operational success remains authoritative zero open orders plus exact flatness (`position.size == "0"`). The sole narrow exception is that the bounded live experiment may stop `FAILED_HALTED_MANUAL_RECOVERY` with only its minimum-size testnet exposure or a known experiment order unresolved after its fixed attempts. That outcome is failure and never operational acceptance, exact-flat acceptance, strategy readiness, or evidence that undocumented semantics generalize.

The current milestone is fixture-only. One isolated RISEx testnet controller uses the direct official REST/EIP-712/ABI contract and an injected existing-session-signer loader that remains uncalled in Builder tests. Before each hypothetical dispatch it durably persists a unique experiment/intent ordinal, nonzero `client_order_id`, reserved bitmap nonce, canonical unsigned payload/action digest, exact order fields, price-evidence identity, and permit deadline. Raw keys, credentials, signatures, wallet cells, and unredacted identity evidence never enter Git, fixtures, ordinary logs, or reports. Timeout, disconnect, or ambiguity never replays an intent or reuses its client ID or nonce.

Fixture preflight requires the exact testnet host, chain, domain and Router identity; active preserved signer status; active BTC perpetual; fresh official tick, step, minimum, BBO and depth; two consistent zero-open-order and exact-flat reads; no unfinished intent or unexplained account state; and an exact current minimum whose 30 bps adverse-bound notional is `<= USD 500`. Failure stops before signer loading, signing or dispatch. The fixed 30 bps bound is a test-only protection parameter and changes no paper or strategy economics.

The hypothetical dispatch contract permits exactly one minimum-size price-bounded `MARKET+FOK` opening and at most three state-based closes. The first close uses the fresh authoritative opposite-side exact position size, `reduce_only=true`, and price-bounded `MARKET+FOK`; this is an empirical official API/current-UI-compatible contract, not guaranteed semantics. A later close may use only a fresh-position-derived official-UI-equivalent price-bounded `LIMIT+IOC`; every new close requires the prior intent to be terminal/reconciled and fresh position/BBO evidence. Only an exact known experiment order may receive one cancel dispatch, and each such order ID is cancelled at most once. No blind retry, account-wide cancellation, unrelated-order mutation, quantity enlargement, or rounding is permitted.

Reconciliation uses the durable client/order identity, order response/by-ID/open/history, trades/fills, authoritative position, permit expiry, and fresh official state. A no-fill ending flat is safe but does not prove the close lifecycle. Success requires an observed lifecycle fill, every intent terminal/reconciled, zero open orders, and exact flatness. Non-step residual, unexplained position/order state, exhausted closes, unresolved exact cancel, connectivity loss, or nonzero final position stops all automated writes and persists redacted `FAILED_HALTED_MANUAL_RECOVERY` evidence plus local operator recovery instructions.

Builder CI uses injected fixtures, synthetic signer values, disposable storage, and no live transport. This milestone has no production credential invocation, protected main-wallet XLSX access, real signing, private/live request, nonce consumption, POST, order, cancel, fill, close, position change, CLI/live-smoke path, shared Scanner/runtime/repository/economics/strategy/Telegram edit, or cross-venue edit. Builder creation needs a separate Chief Coordinator gate after governance publication; any later credential access or testnet write needs another explicit Chief Coordinator live gate.
