...
Before release, the actual compact production candidate must pass three separate gates (owner revision, 2026-09-07):

- HARD_ENVELOPE_SAFE is a strict invariant: each append respects the fixed envelope, campaign aggregate 1,000,000 records/4 GiB including 100,000 records/512 MiB terminal reserve. Preserve the first valid resource failure and a readable/replayable resource-limited prefix; incomplete runs cannot become complete/profitable. Corruption, truncation and time travel fail closed. No cap increase for PASS.
- SEMANTIC_REPLAY_SAFE requires all-D1 streaming BOOK reconstruction including all levels, session/recovery/revision/link/gap identity; two equal canonical replays matching accepted S1b new-policy semantics (decisions, admissions, fills, actions, reservations, positions, exact notional, fees, PnL, marks, reasons and timing). Explain representation-only differences and preserve original transport failure, DATA_INSUFFICIENT and funding UNKNOWN. New numerical format uses canonical semantic Decimal identity; historical evidence/oracles remain immutable.
- CAMPAIGN_PROFILE_FITS applies only to one prospectively frozen conservative research workload on the actual candidate, not all possible market sequences or a universal SLA. Full saved D1 or exact D1 timing/size characteristics overlap with repeated valid NORMAL/flat S1b synthetic episodes on a shared relative t=0 timeline; all four alternatives execute their candidate path. Synthetic episodes are load evidence only. Measure input/reconstruction/kernel/actions/results/serialization/durable persistence/terminal finalization together, actual durable records/bytes, max/final backlog, processing/service timings, workload elapsed time, terminal drain and relevant peak memory when feasible. Require positive byte/record headroom, finite normally drained backlog, no hidden retention/policy halt, common-offset invariant verdict and an overload control that FAILS. Bind true/false to exact frozen profile parameters; a universal capacity_proven=true is insufficient.

Use existing FULL/DELTA storage. Remove redundant source books from results through verifiable references to concrete retained reconstructable BOOK state (or an equally minimal safe solution); a hash without durable reconstructable evidence is insufficient. Preserve session/recovery/revision/gap identity, missing/corrupt-reference rejection and replay without RAM-only objects; version the new format and measure actual deep-result bytes. Do not increase terminal_retention_capacity=64 for PASS. Minimal bounded retirement/index state is allowed after establishing what must remain in RAM versus durable storage, preserving duplicate/no-reuse, relevant late events, evidence identity, replay and terminal correctness. A required large new architecture or inability to fit the frozen profile safely is S2_BLOCKED/STOP, not another abstract capacity-method stage.
