...
### Venue evidence and unresolved implementation details

Official references: [SDK getting started](https://apidocs.lighter.xyz/docs/get-started), [API trading](https://apidocs.lighter.xyz/docs/trading), [order types and matching](https://docs.lighter.xyz/trading/order-types-and-matching), [PnL](https://docs.lighter.xyz/perpetual-futures/pnl-and-total-account-value), and [transfers](https://apidocs.lighter.xyz/docs/deposits-transfers-and-withdrawals). The first documents SignerClient and MARKET/IOC bounded integer price; trading distinguishes accepted API requests from execution; matching follows price/time priority and reduce-only rules. The researched transfer documentation establishes no native position-transfer operation. sendTxBatch does not establish cross-account atomicity. Exact SDK version, expiry units, cancellation/status/trade/account schemas and sufficiency of reconciliation must be bound to official source evidence before implementing those paths. Unsupported semantics must produce a concrete blocker rather than an invented protocol or fake live-ready claim.
