...
### Owner-authorized offline A/B/C comparison — 2026-09-09

The owner authorizes a finite same-saved-pilot experiment specified in NEXT_TASK: A unchanged120s fixed exit; B only300s max_hold through existing policy; C_BE_REPRICE_V1 opt-in120s whole-cycle observed break-even exit repricing. A remains the default accepted policy. C uses cumulative actual net cash plus current exact available Lighter SELL notional for the fully balanced, settled remaining pair, includes forthcoming RISEx maker fee and fixed scenario stress exactly once, caps BUY at ask-minus-tick, and requires causal cancellation/new activation/post-only at no more than1Hz. No future proceeds or dust write-off; outside the fully balanced settled domain keep accepted paired-reduction behavior with an explicit limitation. The first-fill deadline and all other safety/economic constraints are unchanged for C. Full formula, decision timing, scope, candidate checks and end-window censoring are frozen in NEXT_TASK before execution. This experiment does not authorize collection/trading or silently change historical results/default policy.
