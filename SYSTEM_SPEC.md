...
Following full independently reconciled paired opening, sample and persist one integer hold from20through300seconds and wait using monotonic elapsed time from confirmation of both filled positions. Then attempt paired closure with reversed sides and reduce_only on both legs, fresh mirrored one-tick source limit pricing and equal receiver bound. Preserve source-resting-before-receiver causal checks. Closure completion can take longer than the sampled hold. The owner permits separate reduce-only market orders for freshly proven cycle residuals after own-order cancellation/reconciliation; the finite implementation bound is one fallback per account, fresh executable-side book price as bound, and existing request/reconciliation deadlines. No blind replay or increasing/reversing positions, no invented minimum exemption, percentage slippage, price widening or retry campaign. Unknown state prevents dependent mutation. Confirmed partial opening skips holding and is eligible only for bounded residual closing after complete reconciliation; ambiguous opening stops safely with evidence.
