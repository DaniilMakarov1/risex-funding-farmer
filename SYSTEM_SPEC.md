...
- For each arm and every horizon, report the exact sum, mean, minimum, gross positive edge, gross negative edge, and count of negative dependence-unit values. Qualification additionally requires a strictly positive exact sum at every horizon. Thus a rare loss cannot be hidden below p05 while making aggregate economics negative. These are conservative unit scores, not additive executable trade PnL or an estimate of profit per hour. No confidence or independence claim follows from the existing 20/50 evidence floors.
...
- Compare only actual active narrow/wide prices for the same eligible trade, never a Cartesian product of unrelated events. A unit is distinct only if all its comparisons are at least one tick in the required direction. Equality without a negative or unresolved comparison is a collision; any mixed/reversed/unresolved comparison fails the corresponding gate. Concentration counts units once, using earliest strict detection and fixed buckets anchored at the recorded sample-start monotonic timestamp.
