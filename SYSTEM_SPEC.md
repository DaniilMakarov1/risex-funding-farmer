...
Only officially documented onboarding is implementable. The RISEx testnet faucet/deposit may be exposed as one explicit unsigned, one-shot operation followed by the official token-balance postcondition. Nado or Extended signed onboarding remains fail-closed when the official contract cannot be used safely without dependency workarounds or when official chain identity cannot be proved. CI is fixture-only. Real secrets, account writes, faucet/deposit calls, and private live queries remain prohibited until Chief accepts the deterministic candidate and separately authorizes the operational step.
