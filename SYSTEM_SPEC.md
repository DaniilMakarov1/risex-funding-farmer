...
SYSTEM_SPEC_VERSION = 4.1
SPEC_STATUS = FIRST_RAW_RESEARCH_SCANNER_IMPLEMENTED

## HCR-10 amendment — bounded agent mainnet execution and private credential exchange

On 2026-09-15 the owner explicitly removed the blanket prohibition on agent mainnet order execution and on API-key exchange between the owner and the agent assigned to that operation. This amendment supersedes earlier owner-local-only, agent-offline-only, no-agent-execution and no-credential-in-task/chat language solely for a finite live operation explicitly bound in NEXT_TASK. Historical statements remain true for the runs they describe and do not grant authority to repeat them.
...
Agent live authority is not an unlimited trading mandate. For HCR-9 it covers one Robinhood Chain Lighter Mainnet PAIRED_OPENING attempt only after the implementation is accepted, exact account/market/direction/quantity/price/time inputs are fixed, the complete operation and possible remaining inventory are shown, and the owner gives an explicit launch instruction. The assigned agent may then authenticate private reads, prepare/sign/dispatch the bounded orders and perform only the already-specified cancellation and reconciliation actions. No series loop, automatic retry, repricing, compensation, transfer, withdrawal, leverage/margin-setting change, new market/size, unrelated cancellation or autonomous strategy is authorized. Ambiguity stops new writes and preserves reconciliation/no-replay semantics. This documentation amendment itself performs no live access or order.

## HCR-8 amendment — explicit operator margin deferral (accepted offline)

Owner may opt into `defer_incremental_margin_calculation:true` for PAIRED_OPENING in direct/series configuration or via `--defer-incremental-margin-calculation`. Default false remains strict. This supersedes mandatory incremental estimate/provenance only for the explicit deferred path; missing evidence stays uncalculated. Supplied nonfinite/negative/malformed estimates remain rejected, evidenced insufficient estimates still block. Existing balance/current-margin, identity/positions/orders, fees, prices, sizes/minima, timing, reconciliation and no-replay rules remain. Bind policy to plan/parent/child journals and reject changed-policy restart. Exchange rejection is not fill/success. Readiness stays read-only and rejects the execution option. Owner chose Mainnet; HCR-10 now permits the assigned agent to perform the exact accepted HCR-9 live operation after its remaining gates are satisfied.
