...
The next implementation is measurement-only. Before the later evaluator is assigned, the Chief must resolve the audited aggregate-loss loophole, mixed/contaminated unit admission, no-fill denominator, and stage fingerprint semantics explicitly in this specification. No numerical observation may choose those rules. The scanner must never claim that p05/median/positive share alone imply positive average economics, or that order-ID grouping proves statistical independence.
