...
- Active implementation task: TELEGRAM-001 — Outbound Runtime Notifications
...
- Telegram: outbound-only milestone authorized; disabled by default; no inbound commands or Telegram-triggered scans
...
TELEGRAM-001 is active without interrupting the existing PAPER-007 Stage B process. The previously disclosed token is compromised and must not be used; only a newly rotated token and chat ID supplied through environment may enable delivery after acceptance.
