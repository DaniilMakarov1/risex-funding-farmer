...
- The subsequent paper-run is operationally blocked. Bootstrap initially covered only 28 Nado directions while Extended catalog loaded later. The first FULL deadline was observed 87.5 seconds late; the all-route refresh issued many concurrent public observations and produced 37–60 second timeouts across RISEx, Nado, and Extended, never persisted a FULL scan, and did not finish SIGTERM shutdown within 30 seconds. Chief force-stopped only the exact process after confirming zero orders, fills, and positions. SQLite integrity is `ok`; this is failure evidence, not a completed window.
