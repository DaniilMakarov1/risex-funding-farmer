...
Public-only research and paper remain the default; points are $0. A finite owner-authorized contract recorded in NEXT_TASK may additionally authorize a named agent to access the specified accounts, use real API credentials, authenticate private endpoints, sign and dispatch explicitly bounded testnet or mainnet orders, and perform the exact cancellation/reconciliation actions required by that contract. Authority never generalizes beyond the recorded venue, environment, accounts, market, direction, quantity, price bounds, time bounds, attempt count and recovery behavior. Missing exact inputs or launch authority blocks mutation; an authorized agent must show the final plan and obtain the owner's explicit launch instruction before the first live write. No autonomous strategy execution, automatic retry, unrelated cancellation, transfer, withdrawal, leverage/margin-setting change or compensating trade follows from this exception.
