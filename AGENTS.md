...
Public-only research, paper default, points $0. A finite owner-authorized development contract in NEXT_TASK may permit writing a separately isolated opt-in utility for later user operation; writing and offline mock testing that utility does not authorize agents to access accounts, handle real credentials, prepare real signed payloads or execute its live path. Private/account/fee-reader endpoints, credentials, signing, order preparation/dispatch, testnet/mainnet orders, funds, transfers, withdrawals and strategy execution are prohibited. Secrets never enter chat, arguments, logs, reports, databases, fixtures or Git. Existing private/testnet modules remain isolated and uninvoked. Public collection needs an explicit prospective gate in NEXT_TASK; old smoke/pilot/campaign permissions are not reusable.
