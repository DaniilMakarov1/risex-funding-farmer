...
- Assign each new finite objective to a fresh Builder task/session with a compact current contract, not an accumulated prior-task conversation. Keep corrections to the same objective in its healthy session when state remains clear. If fresh creation is unavailable, report the concrete limitation; do not silently reuse a historical Builder. Context compaction alone is not evidence of drift: perform a bounded recovery against current contract, actual diff and evidence when the owner requests it or a concrete inconsistency appears. Preserve WIP and stop predecessor writes before any ownership transfer.
...
- Changes to arithmetic, positions, execution, causal ordering or data/result preservation require distinguishing adverse regressions, relevant integration checks and one final clean isolated Python 3.11 full suite before acceptance. Preserve credential containment, funding, no-replay/reconciliation and exact-flat regressions. Expected numerical results must be computed independently of the tested math helper.
...
Public-only research and paper remain the default unless NEXT_TASK records an owner-authorized Mainnet or testnet operation. The owner permits a named execution agent to access the specified accounts, use real API credentials, authenticate private endpoints, sign, dispatch and reconcile orders when the owner has explicitly confirmed that concrete action. Confirmation can be a plain affirmative response to the agent's concise proposal; the agent does not need to recite a fixed checklist of venue/account/market/order fields unless the owner asks for it or the implementation needs the value. A confirmation authorizes the proposed action and its necessary reconciliation/cancellation handling. Completion, cancellation, a materially different action, or unclear owner intent requires a fresh confirmation before new writes. Missing inputs needed by the implementation still block mutation. This does not authorize withdrawals, transfers, unrelated account-wide cancellation, leverage/margin-setting changes, hidden autonomous strategy changes, or blind replay of an ambiguous write.

The owner may provide API credentials to the assigned agent through a private owner-agent task/chat or protected local credential boundary, and may explicitly request that the assigned agent return the same credential in that private conversation. Default responses mask credentials. Credentials may be held only as long as needed for the authorized operation and may be stored only in the approved protected credential store. They must never enter Git/GitHub, commits, patches, tracked or untracked project files, command arguments, environment variables, process titles, reports, fixtures, evidence packets, terminal captures, tool output intended as durable evidence, public shares, or messages to any person or agent not assigned to the operation. Seed phrases, wallet recovery material and withdrawal credentials remain prohibited. Existing private/testnet modules remain isolated unless the active contract explicitly names their use. Public collection still needs an explicit prospective gate in NEXT_TASK; old smoke/pilot/campaign permissions are not reusable.
