...
- Do not create milestones beyond BOOTSTRAP-000 and PAPER-001 through PAPER-006, except the explicitly authorized PAPER-007 experiment, TELEGRAM-001 outbound-notification work, bounded TELEGRAM-001-FIX-001 correction, TELEGRAM-002 full-scan digest and its bounded two-decimal display correction, PAPER-007-FIX-004 public REST timeout correction, PAPER-007-FIX-005 first-full funding-freshness correction, PAPER-007-FIX-006 RISEx checksum-resubscribe correction, PAPER-007-FIX-007 Extended expected/applied funding and socket-health separation, PAPER-007-FIX-008 public evidence/catalog/heartbeat/digest/stop correction, PAPER-007-FIX-009 public stream/time/startup consistency correction, and the separately governed `TESTNET-002-RISEX-ORDER-LIFECYCLE-001`, `EXTENDED-TESTNET-001`, and `NADO-TESTNET-001` bounded lifecycle cores. The bounded RISEx credential/private-read preflight in `NEXT_TASK.md` remains part of the existing RISEx lifecycle and is not a new product milestone.
...
- The user's explicit testnet decision authorizes accepted `TESTNET-001` account bootstrap/read-only connectivity, the accepted `TESTNET-002-RISEX-SIGNER-001` session signer, the centrally accepted fixture core of `TESTNET-002-RISEX-ORDER-LIFECYCLE-001`, the accepted Extended Phase 0 plus `EXTENDED-TESTNET-001`, and the accepted Nado Phase 0 plus `NADO-TESTNET-001` recorded in `NEXT_TASK.md`. RISEx may advance only through the bounded credential/private-read preflight recorded there: two public sweeps first, then one existing-signer `RegisterV2`/`auth_v2` private-read attempt only after a separate Chief Coordinator operational gate. This governance candidate itself authorizes no credential access, nonce, real signature, private request, Builder, or live write. Extended and Nado remain within their separately governed fixture-only boundaries. A blocked slice authorizes no Builder or private/executing write. This does not authorize mainnet, real funds, Scanner/runtime strategy integration, or operational readiness inferred from unknown semantics.
...
- Extended and Nado Phase 0 are accepted, and `EXTENDED-TESTNET-001` and `NADO-TESTNET-001` are their active fixture-only slices. No venue Builder may start until that venue's governance candidate is centrally accepted and published to `main` and Chief Coordinator separately authorizes the sole Builder.
...
- Paper remains the default product. The completed one-shot RISEx testnet signer registration and centrally integrated RISEx fixture lifecycle core are accepted history. The active RISEx step is only the bounded preflight in `NEXT_TASK.md`: public reads precede any secret boundary, and actual credential loading, auth nonce, `RegisterV2` signing, or private WebSocket read still requires a separate Chief Coordinator operational gate. It permits no order/cancel/close POST or other live write; those require a later separate Chief Coordinator live-write gate. Mainnet, real funds, and strategy execution remain prohibited.
