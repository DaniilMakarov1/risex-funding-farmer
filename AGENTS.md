...
- The user's explicit testnet decision authorizes `TESTNET-001` account bootstrap/read-only connectivity and its bounded `TESTNET-001-RECOVERY-001` safety correction. This exception does not authorize testnet orders in the current slice, mainnet, real funds, Scanner/runtime integration, or any trading endpoint. Later testnet order/cancel/close work requires a separately governed `TESTNET-002` after TESTNET-001 acceptance.
