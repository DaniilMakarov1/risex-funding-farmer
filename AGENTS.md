...
Public-only research and paper remain the default; points are $0. A finite owner-authorized contract recorded in NEXT_TASK may additionally authorize a named agent to access the specified accounts, use real API credentials, authenticate private endpoints, sign and dispatch explicitly bounded testnet or mainnet orders, and perform the exact cancellation/reconciliation actions required by that contract. For every live operation, the agent must first render the exact venue, environment, accounts, market, side/direction, order types, quantities, price bounds, time bounds, maximum write count, expected resulting positions and permitted recovery actions, then ask the owner for confirmation of that specific plan. The owner's affirmative response authorizes the agent to execute that plan and its listed recovery actions once. Material plan/state changes, completion, cancellation or a new operation require a new rendered plan and confirmation; a prior confirmation is never standing or transferable authority. Missing exact inputs or confirmation blocks mutation. No autonomous strategy execution, implicit retry, unrelated cancellation, transfer, withdrawal, leverage/margin-setting change or unconfirmed compensating trade follows from this procedure.
