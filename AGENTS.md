...
- The user's explicit testnet decision authorizes `TESTNET-001` account bootstrap/read-only connectivity and its bounded safety corrections, including `TESTNET-001-RECOVERY-005` direct passwd-home one-shot state. This exception does not authorize testnet orders in the current slice, mainnet, real funds, Scanner/runtime integration, or any trading endpoint. Later testnet order/cancel/close work requires a separately governed `TESTNET-002` after TESTNET-001 acceptance.
...
- Paper remains the default product. The sole current exception is the narrowly governed testnet-only account bootstrap/read-only connectivity in `NEXT_TASK.md`; no mainnet, real funds, trading orders, or strategy execution is permitted by that exception.
